...
-- Plan: 536 rows (535 online, 1 coupon), generated from ug_payment_mode_plan.csv.
--
-- HOW TO RUN IT
...
--   Step 1 copies the before-state into a real table (ug_payment_mode_backup_20260918)
...
--   Keep ug_payment_mode_backup_20260918 until you are happy. Do not drop it.
--
-- DO NOT remove the SET LOCAL app.skip_automation line. Without it the
-- automation trigger fires on all 536 students and can send them mail.
-- ===========================================================================
...
-- ---------------------------------------------------------------------------
CREATE TABLE ug_payment_mode_backup_20260918 AS
...
DECLARE n int;
BEGIN
  SELECT count(*) INTO n FROM ug_payment_mode_backup_20260918;
  IF n <> 536 THEN
    RAISE EXCEPTION 'Snapshot holds % rows, expected 536. Nothing has been changed. Rebuild the plan and start again.', n;
  END IF;
  IF EXISTS (SELECT 1 FROM ug_payment_mode_backup_20260918 WHERE payment_mode_before IS NOT NULL) THEN
...
BEGIN
  SELECT count(*) INTO n
    FROM ug_payment_mode_backup_20260918 b JOIN v2_leads l ON l.id = b.v2_lead_id
   WHERE l.payment_mode = b.payment_mode_after;
  IF n <> 536 THEN
    RAISE EXCEPTION 'Only % of 536 rows carry the new value. Rolling back.', n;
  END IF;
END
$guard$;
...
-- ---------------------------------------------------------------------------
CREATE TABLE ug_payment_mode_timelines_20260918 AS
WITH ins AS (
...
DECLARE n int;
BEGIN
  SELECT count(*) INTO n FROM ug_payment_mode_timelines_20260918;
  IF n <> 536 THEN
    RAISE EXCEPTION 'Wrote % timeline rows, expected 536. Rolling back.', n;
  END IF;
END
$guard$;
...
BEGIN
  SELECT count(*) INTO n
   ...
   WHERE table_name = 'v2_leads'
     AND created_at >= now()
     AND row_id IN (SELECT v2_lead_id FROM ug_payment_mode_backup_20260918);
  IF n <> 0 THEN
